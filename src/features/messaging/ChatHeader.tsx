...
export function ChatHeader({ name, avatar, tone, actions }: { name: string; avatar: string | null; tone: "rose" | "gold"; actions?: React.ReactNode }) {
  const { t } = useI18n();
  return (
    <div className="relative z-20 flex items-center gap-3 px-2 pt-[max(0.6rem,env(safe-area-inset-top))] pb-2.5">
      <Link href="/messages" aria-label={t("common.back")} className="icon-btn shrink-0 text-muted hover:text-ink">
        <AppIcon name="back" size={22} />
      </Link>
      <Avatar path={avatar} tone={tone} size={38} className="shrink-0" />
      <span className="font-display text-xl leading-none flex-1 min-w-0 truncate">{name}</span>
