...
export function ChatInfoButton({ name, avatar, tone, photos }: { name: string; avatar: string | null; tone: "rose" | "gold"; photos: string[] }) {
  ...
  return (
    <>
      ...
      {open && (
        <Portal>
          <div role="dialog" aria-modal="true" aria-label={t("messaging.info")} className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-5" onClick={() => setOpen(false)}>
