...
export function CallHistorySheet({ myId, onClose }: { myId: string; onClose: () => void }) {
  ...
  return (
    <Portal>
      <div role="dialog" aria-modal="true" aria-label={t("call.history")} className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center" onClick={onClose}>
            ...
            <ul className="grid gap-1.5">
              {rows.map((c) => (
                <li key={c.id} className="flex items-center gap-3 rounded-2xl bg-surface2 px-3.5 py-2.5">
                  ...
                  <button type="button" className="icon-btn shrink-0" disabled={inCall} aria-label={t("call.redial")} onClick={() => { onClose(); startCall(c.kind); }}>
