...
function DropMenu({ trigger, label, items, className, chevron = true }: { trigger: React.ReactNode; label: string; items: Item[]; className: string; chevron?: boolean }) {
  ...
  return (
    <div ref={box} className="relative">
      <button type="button" className={className} aria-label={label} aria-haspopup="menu" aria-expanded={open} onClick={() => setOpen((o) => !o)}>
        {trigger}
        {chevron && <AppIcon name="down" size={14} className={`transition-transform duration-200 ${open ? "rotate-180" : ""}`} />}
      </button>
      {open && (
...
export function ChatHeaderMenus({ myId, name, avatar, tone }: { myId: string; name: string; avatar: string | null; tone: "rose" | "gold" }) {
  ...
  return (
    <>
      <DropMenu
        label={t("call.menu")}
        className="btn !min-h-10 !px-3 gap-1.5 text-sm shrink-0"
