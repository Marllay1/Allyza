...
export default async function SharingPage() {
  ...
  return (
    <>
      <PageHeader title={t("settings.sharing")} subtitle={t("sharing.subtitle")} back="/settings" backLabel={t("common.back")} />
      <SharingForm initial={initial} herPartnerLinked={!!v.couple?.partnerId} />
      <p className="text-xs text-muted text-center mt-6 text-balance">{t("sharing.footer")}</p>
    </>
  );
}
